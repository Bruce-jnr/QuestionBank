...
export default function ArticlePage() {
  ...
  async function upvote(id) {
    ...
    try {
      ...
      setComments((items) => items.map((item) => item.id === id ? { ...item, upvotes: item.upvotes + 1 } : item))
    } catch (requestError) {
      setError(requestError.message)
    } finally {
      setUpvotingId(null)
  ...
  return (
    <PublicLayout>
      <article className="article-page">
        ...
        <h1>{post.title}</h1>
        <div className="article-meta">By {post.author_name || 'CBRUCENCLEX'} | {formatDate(post.published_at)}</div>
        ...
        <section className="comments-section"><h2>Join the discussion</h2><form className="stacked-form" onSubmit={submitComment}><div className="form-row"><input required placeholder="Your name" value={form.author_name} onChange={(event) => setForm({ ...form, author_name: event.target.value })} /><input placeholder="Email" type="email" value={form.author_email} onChange={(event) => setForm({ ...form, author_email: event.target.value })} /></div><textarea required placeholder="Write a comment" value={form.content} onChange={(event) => setForm({ ...form, content: event.target.value })} /><button aria-busy={submitting} disabled={submitting} type="submit"><ButtonLoader loading={submitting} loadingText="Submitting...">Submit comment</ButtonLoader></button></form>{error && <p className="form-note">{error}</p>}<div className="comment-list">{comments.map((comment) => <article key={comment.id}><strong>{comment.author_name}</strong><p>{comment.content}</p><button aria-busy={upvotingId === comment.id} disabled={upvotingId !== null} onClick={() => upvote(comment.id)} type="button"><ButtonLoader loading={upvotingId === comment.id} loadingText="Saving...">Helpful {comment.upvotes || 0}</ButtonLoader></button></article>)}</div></section>
      </article>
    </PublicLayout>
  )
}
